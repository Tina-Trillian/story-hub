...
class NewStory extends React.Component {
  ...
  render() {
    ...
    //beginning
    if (NewStoryStore.genreList &&
      NewStoryStore.allGenre.length === Object.keys(NewStoryStore.genreList).length)
      genreList = NewStoryStore.allGenre.map((el, index) => {
      return ( <span key={`genr_${index}`}><input
        name={el}
        onChange={(evt) =>
    ...
    return (
      <div className="container">
        ...
        <input
          name="public"
          onChange={(evt) => NewStoryStore.handleInputChange("is_public", evt.target.checked)}
          type="checkbox"
        ...
        <input
          name="is_moderated"
          onChange={(evt) => NewStoryStore.handleInputChange("is_moderated", evt.target.checked)}
          type="checkbox"
        ...
        onChange={evt => NewStoryStore.handleInputChange("temp", evt.target.value)}
        placeholder="Type a tag here" />
        <button
        onClick={() => NewStoryStore.addTag()}>Add Tag</button>
        ...
        <input type="file"
        className="input"
        onChange={evt => NewStoryStore.handleInputChange('picture', evt.target.files[0])}
        placeholder="Set the mood ..." />
        <br />
        <br />
        <button onClick={ () =>
           NewStoryStore.createNewStory()
        }>Create your story and write the first Part!</button>
      </div>
    );
  }
}
